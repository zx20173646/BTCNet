...
def huffman_encoder_BTCNet(acquisition, directory='HuffmanEncodings/'):
    os.makedirs(directory, exist_ok=True)
    original_total = 0
    compressed_total = 0

    # acquisition = acquisition.cpu().numpy()
    shape = acquisition.shape
    form = 'csr' if shape[0] < shape[1] else 'csc'
    mat = csr_matrix(acquisition) if shape[0] < shape[1] else csc_matrix(acquisition)

    # Encode
    t0, d0 = huffman_encode(mat.data, f'_{form}_data', directory)
    t1, d1 = huffman_encode(mat.indices, f'_{form}_indices', directory)
    t2, d2 = huffman_encode(mat.indptr, f'_{form}_indptr', directory)

    # caculate original and compressed data quantity
    original = mat.data.nbytes + mat.indices.nbytes + mat.indptr.nbytes
    compressed = t0 + t1 + t2 + d0 + d1 + d2

    # Print compression rate
    print(f" {original:10} {compressed:10} {original / compressed:>10.2f}x    {100 * compressed / original:>6.2f}%")

    return 100 * compressed / original

def huffman_decode_BTCNet(acquisition, directory='HuffmanEncodings/'):
    # acquisition = acquisition.cpu().numpy()
    shape = acquisition.shape
    form = 'csr' if shape[0] < shape[1] else 'csc'
    ...
    # Decode data
    data = huffman_decode(directory, f'_{form}_data', dtype='float32')
    indices = huffman_decode(directory, f'_{form}_indices', dtype='int32')
    indptr = huffman_decode(directory, f'_{form}_indptr', dtype='int32')

    # Construct matrix
    mat = matrix((data, indices, indptr), shape)

    decoded = mat.toarray()
    # decoded = torch.from_numpy(mat.toarray())
