...
def trainer():
    traindata = loadTxt('trainpath/train.txt')
    testdata = loadTxt('testpath/test.txt')

    train_loader = torch.utils.data.DataLoader(dataset_h5(traindata, width=WIDTH, marginal=60, root=''), batch_size=batch_size, shuffle=True, num_workers=16)
    ...
    model = BTCNet(bit_num=10, cr=CR).to(device)
