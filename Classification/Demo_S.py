...
save_path = 'checkpointSalinas/CNet_on_Corrected_%s512x128_OA%s'  # the prefix of saved trained checkpoint
# trained checkpoint
state_dict = torch.load('checkpointSalinas/CNet_on_Corrected_salinas512x128_OA0.9820110553727194')


for (FLAG,curr_train_ratio) in [(0, 0.1)]:  # train sample ratio: 10%
    OA_ALL = []
    AA_ALL = []
    KPP_ALL = []
    AVG_ALL = []
    # trained checkpoint seed: 0, 2, 4, 7, 9, 10, 11, 13, 15, 17
    Seed = range(0, 1, 20)  # the range of random seed
    #  The random seeds corresponding to the 10 trained checkpoints are recorded in logSalinas/S_ori.txt
    #  In the annotation # after each experimental result.
    Seed_List = [4]  # current seed value

    if FLAG == 0:
        data_mat = sio.loadmat('Datasets/Salinas/Salinas512x128.mat')   # original data
        # data_mat = sio.loadmat('Datasets/Salinas/RecSalinas512x128-BTC-Net.mat') # Rec data by BTC-Net
        # data_mat = sio.loadmat('Datasets/Salinas/RecSalinas512x128-DCSN.mat')
        # data_mat = sio.loadmat('Datasets/Salinas/Recsalinas512x128-PCA+JPEG2K.mat')
        # data_mat = sio.loadmat('Datasets/Salinas/RecSalinas512x128-E3DTV.mat')
        data = data_mat['data']
        # data = data_mat['salinas']

        gt_mat = sio.loadmat('Datasets/Salinas/CroppedSalinasGt.mat')  # 512 x 128 ground truth
        gt = gt_mat['gt']
    ...
    model = ClassifierS(class_count, n_bands, 150)
    ...
    if mode == '1':  # train mode
        ...
        for eep in range(max_epoch):
            ...
            if eep == 260:
                ...
                Draw_Classification_Map(OutputWhole,
                                        'ResultsImage/' + dataset_name + '_CC2B_' + str(train_ratio) + '_' + str(OA))
        ...
        OutputWhole = PatchStack(Output, m, n, patch_height, patch_width, split_height, split_width, EDGE,
                                 class_count + 1)
        print(np.shape(OutputWhole))
        sio.savemat('Datasets/Salinas/PredictionRecSalinas.mat', {'gt': OutputWhole})
        AC, OA, AA, _, _ = ClassificationAccuracy(OutputWhole, Test_Label, class_count + 1)
        kappa = Kappa(OutputWhole, Test_Label, class_count + 1)
